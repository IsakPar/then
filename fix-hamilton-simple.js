...
// Hamilton Seat Mapping Fix - Simple SQL Version
// This script creates ALL hardcoded seat mappings for Hamilton using raw SQL
...
const connectionString = 'postgresql://postgres:ZllsKKUNzfOtPYgAqNYqNTlLkqVPNlVV@junction.proxy.rlwy.net:36039/railway';

async function main() {
  console.log('🎭 Starting Hamilton seat mapping fix (Simple SQL version)...');
  ...
  try {
    ...
    const hamiltonSeats = await client`
      SELECT 
        s.id as seat_id,
    ...
    // Step 4: Insert all mappings in batches using SQL
    ...
    for (let i = 0; i < mappingsToCreate.length; i += batchSize) {
      ...
      // Create SQL VALUES string for batch insert
      const values = batch.map(mapping => 
        `('${mapping.showId}', '${mapping.hardcodedSeatId}', '${mapping.realSeatId}')`
      ).join(', ');
    ...
    const testMapping = await client`
      ...
      WHERE hm.show_id = ${HAMILTON_SHOW_ID} AND hm.hardcoded_seat_id = 'sideA-1-2'
    `;
    ...
    if (success) {
      const mapping = testMapping[0];
      console.log(`   ✅ sideA-1-2 → ${mapping.section_name} Row ${mapping.row_letter} Seat ${mapping.seat_number}`);
    }
