-- Complete London Shows Migration with Local Images
-- This migration sets up the complete database schema and data for London West End shows
-- Total capacity: 1000 seats per venue
-- Available seats: 150-300 per show
-- Price range: £20-£90

-- Clean up existing data (if any)
DELETE FROM show_section_pricing WHERE show_id IN (SELECT id FROM shows WHERE venue_id IN (1,2,3,4,5,6));
DELETE FROM seat_bookings WHERE show_id IN (SELECT id FROM shows WHERE venue_id IN (1,2,3,4,5,6));
DELETE FROM shows WHERE venue_id IN (1,2,3,4,5,6);
DELETE FROM venue_sections WHERE venue_id IN (1,2,3,4,5,6);
DELETE FROM venues WHERE id IN (1,2,3,4,5,6);
...
(6, 'Apollo Victoria Theatre', '17 Wilton Rd, Pimlico, London SW1V 1LG', 1000, 51.4956, -0.1441, 'apollo-victoria-theatre', null, null);
...
(6, 'Stalls', 400, 1.0, 1),
(6, 'Dress Circle', 300, 1.2, 2),
(6, 'Upper Circle', 300, 0.8, 3);

-- Create popular London shows with local images
...
(6, 'Wicked', 6, NOW() + INTERVAL '2 days 3 hours', 58.00, 'The untold story of the Witches of Oz', '/wicked.jpeg', 51.4956, -0.1441);
...
DECLARE
    ...
    sold_seats INTEGER;
BEGIN
    -- Delete existing pricing data first
    DELETE FROM show_section_pricing WHERE show_id IN (1,2,3,4,5,6);
...
DROP POLICY IF EXISTS "Public venues access" ON venues;
DROP POLICY IF EXISTS "Public shows access" ON shows;
DROP POLICY IF EXISTS "Public sections access" ON venue_sections;
DROP POLICY IF EXISTS "Public pricing access" ON show_section_pricing;
DROP POLICY IF EXISTS "Public bookings access" ON seat_bookings;
DROP POLICY IF EXISTS "Authenticated booking insert" ON seat_bookings;
DROP POLICY IF EXISTS "Authenticated booking update" ON seat_bookings;
...
SELECT 'Venues created:' as info, COUNT(*) as count FROM venues WHERE id IN (1,2,3,4,5,6);
SELECT 'Sections created:' as info, COUNT(*) as count FROM venue_sections WHERE venue_id IN (1,2,3,4,5,6);
...
-- Show pricing summary with local images
SELECT 
    s.name as show_name,
...
GROUP BY s.id, s.name, v.name, s.start_time, s.image_url
